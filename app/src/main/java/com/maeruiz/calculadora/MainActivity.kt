...
//Utilizo esta variable para calcular el resultado, solo se hace  para que sea mas entendible lo del codigo, por que no hace falta utilizar esta variable
...
when (v?.id) {
    ...
    R.id.btndel -> {
        ...
        if ( (cadena != null) && (!cadena.equals("")) ) { //Verificar que la cadena no este vacia o nula
            cadena = cadena.substring(0, cadena.length - 1) //Se elimina  el ultimo caracter
